...
export default function BagSalesApp() {
  ...
  const [settings, setSettings] = useState({ businessName: "My Bag Business", phone: "", address: "" });
      ...
      try {
        ...
        if (res && res.value) {
          ...
          if (!cancelled) {
            ...
            setSettings(data.settings || { businessName: "My Bag Business", phone: "", address: "" });
            setLoaded(true);
            return;
          }
        }
      } catch (e) { /* no saved data yet — start empty, this is expected on first run */ }
      if (!cancelled) {
        setProducts([]); setCustomers([]); setOrders([]); setReminders([]);
        setSettings({ businessName: "My Bag Business", phone: "", address: "" });
        setLoaded(true);
      }
    }
    load();
